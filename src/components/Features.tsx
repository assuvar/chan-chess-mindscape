...
const Features = () => {
  ...
  return (
    <section ref={sectionRef} className="py-24 bg-white relative overflow-hidden text-foreground">
        ...
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {features.map((feature, index) => {
            ...
            return (
              <Card 
                key={index}
                className="feature-card group relative overflow-hidden border-2 bg-card p-8 transition-all duration-300 hover:-translate-y-2 hover:shadow-xl hover:shadow-primary/20 text-foreground"
              >
                <div className="absolute inset-0 bg-gradient-to-br from-primary/5 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
                
                <div className="relative space-y-4 text-foreground">
